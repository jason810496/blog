...
這次參加 [Apache CommunityOverCode Asia](https://asia.communityovercode.org/) 後，很明顯地看到亞洲，尤其是中國大型企業，長出一套跟北美常見雲端資料架構不太一樣的開源技術路線。
...
這背後不是單純的「技術偏好不同」。現場交流、案例分享和攤位資料來看，某些專案的使用者分布也明顯集中在中國境內；中國地區以外的使用者能見度相對低很多。
...
開源專案可以放在 Apache，也可以用英文文件、GitHub issue 和 mailing list 運作，但實際使用者、貢獻者、商業公司和最佳實踐，仍然可能高度集中在某個語言、產業或區域裡。Apache 的品牌是全球的，技術採用卻不一定平均地全球化。雖然也都有按照著 "The Apache Way" 進行，但在 CoC Asia 多數的用戶討論和開發貢獻**都是在微信群組以中文討論**。
...
這套系統不是單點優化，而是端到端地處理 data pipeline 的生命週期。它會區分 release process、maintaining mode、managed process 和 not managed process；也會處理上游系統無法取得 metrics 時，整條 pipeline 要怎麼觀測和保護。底層還有 SQL layer、compute interface，以及把同一套語意 compile 到 [Flink](https://flink.apache.org/)、[Spark](https://spark.apache.org/docs/latest/) 等不同 compute engine 的設計。

它讓我印象深的是「SLA」在這裡不是單純的告警規則，而是系統設計的中心。從 bare metal 分級、fault tolerance、master backup、hot backup、cold backup、switch backup，到整條 pipeline metrics 的 observation 和 auto recovery，全部都圍繞一件事：data pipeline 不能只是能跑，還要能被保證。
...
在 CoC Asia 中，我更明顯感覺到 [Ray](https://docs.ray.io/en/master/ray-overview/index.html) 在 AI / ML workload 裡的位置。雖然是在 Apache 的年會，但它 (目前屬於 [PyTorch Foundation](https://pytorch.org/foundation/)) 還是在多個 session 被反覆提到。
...
以我對 Apache CommunityOverCode North America 的印象，Streaming Compute Engine 並不是那麼核心的主軸；但在 Asia，[Apache Flink](https://flink.apache.org/)、[Apache Pulsar](https://pulsar.apache.org/docs/)、message queue、streaming join、streaming warehouse 這類議題會很自然地出現在主舞台上直接反映業務場景本身的差異。
...
很多大型網路服務的核心問題本來就不是單表分析，而是關係網路：使用者、商品、主播、內容、交易、設備、IP、風控事件、推薦鏈路，全部都互相連在一起。當資料關係本身很複雜，而且還要即時處理時，graph 和 streaming 就很自然地被放在一起討論。

![stream-graph-computing-talk](stream-graph-computing-talk.jpeg)
![stream-join-operator-slide](stream-join-operator-slide.jpeg)

這次有一個有趣的議程是 [Accelerating Multi-stream Join by Stream Graph Computing](https://asia.communityovercode.org/sessions/streaming-907134.html)。它談的是用 Stream Graph Computing 加速多流 join，也就是把 streaming join 的問題轉成圖上的增量計算問題來看。當業務問題本質上是關係和路徑，graph compute engine 就成為 real-time feature、風控規則和推薦系統的一部分。
...
> ![Fluss](fluss.jpeg)
> 題外話，在議程最後還現場見識 [Kafka](https://kafka.apache.org/documentation/) PMC [嘉平](https://github.com/chia7712) 老大詢問 [Fluss](https://fluss.apache.org/) 之於 [Kafka](https://kafka.apache.org/documentation/) 的優勢中，要如何以 [Kafka](https://kafka.apache.org/documentation/) 的角度解決。
...
我在 2025 年 7 月是第一次看到這個專案；但到寫這篇文章的 2026 年 5 月，已經在 LinkedIn、YouTube 上看到越來越多關於 [Fluss](https://fluss.apache.org/) 的文章和影片，也是到目前在國際上比較會看到的中國企業開源 Apache 專案 (如果撇除開源模型來說，只看跟 Data Infra 有關的專案的話)

![kafka-4-highlights-talk](kafka-4-highlights-talk.jpeg)

最後當然也要參加 OpenSource4You 的 [#apache-kafka 頻道](https://opensource4you.slack.com/archives/C06MSQ9V4F3) 的議程。平常都淺水在頻道內，這次看到 [JiunnYang Huang](https://github.com/m1a2st)、[PoAn Yang](https://github.com/FrankYang0529)、[TengYao Chi](https://github.com/frankvicky)、[KuanPo Tseng](https://github.com/brandboat) 現場分享 [Kafka 4.0](https://kafka.apache.org/40/documentation.html) 的重要貢獻，還有最後 [ChiaPing Tsai](https://github.com/chia7712) 在 Fluss 議程現場與講者討論 streaming 場景的核心痛點該如何解決，特別精彩。
...
這次議程裡有多場把 OSS、商業化和 startup 放在一起談的議程。Day 2 早上我聽了其中兩場：
...
這中間的差異不是社群活不活躍，而是企業是否把開源當成正式工程策略。台灣有很強的社群文化，但中國大型企業有更大的商業場景、更大的服務規模，以及更多被公司正式分配到開源維護上的人力。這些條件疊在一起，才會讓開源從**活動文化變成產業級的工程投入**。
...
這也是我在 Apache CommunityOverCode Asia 看到當企業、社群、學生和 ASF 真的接在一起時，可以形成多大的推力。這也是 [源來適你](https://github.com/opensource4you/readme) 一直以來在推動的（再次感謝嘉平！）

## TAC 夥伴

雖然我這次沒有申請到 Apache CoC Asia 的 TAC (Traval Assistance Committee) 補助，但大多時間也是待在 Apache 的攤位順便幫忙發發貼紙。

這次也很開心能跟 Owen、Peter、Ryan、Simon、Nick 這些 TAC 夥伴一起在會場交流。很多時候參加國際會議最有價值的不是單向聽 talk，而是在議程之間、吃飯時、會後聚會裡，跟不同地方的工程師交換對開源、職涯、專案維護的看法。
...
嘉平也揪了有在 OpenSource4You [#apache-kafka](https://opensource4you.slack.com/archives/C06MSQ9V4F3)、[#apache-gravitino](https://opensource4you.slack.com/archives/C07473LAC15) 頻道中剛好在北京夥伴一起吃飯，聊了很多關於 Apache 專案、社群、企業的話題。看到真實的網友見面會也是很有趣的經驗，尤其是當你平常在網路上看到他們參與貢獻，這次終於在現場看到本人。
